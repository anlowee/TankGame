...
void DiamondSquare::_on_start()
{
    ...
    map[0][0]=double(rand()) / RAND_MAX * range;
    map[0][size-1]=double(rand()) / RAND_MAX * range;
    map[size-1][0]=double(rand()) / RAND_MAX * range;
    map[size-1][size-1]=double(rand()) / RAND_MAX * range;
    //这四个初始值可以改
}
